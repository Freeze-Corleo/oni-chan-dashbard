...
const Restaurant = () => {
  ...
  const addProduct = (img: string, name: string, price: number) => {
    setId(id + 1);
    dispatch(
      dispatchAddProduct({ id: id, img: img, name: name, price: price })
    );
  };
            ...
            <div className="grid grid-cols-3 place-items-center gap-10 mb-16">
              {rest.data.product?.data?.products?.map((plat: any) => (
                <>
                  <button
                    onClick={(ho) =>
                      handleOpen(plat.imageUrl, plat.title, plat.price)
                    }
                  >
                    <MenuCard img={plat.imageUrl} label={plat.title} />
                  </button>
                  <Modal
                    open={open}
                    onClose={handleClose}
                    ...
                    <Box sx={style}>
                      <div className="grid grid-cols-1 place-items-center mt-10">
                        <img
                          ...
                          src={process.env.PUBLIC_URL + datas.img}
                        ...
                        <Button
                          variant="outlined"
                          onClick={(price) =>
                            addProduct(datas.img, datas.title, datas.price)
                          }
                        >
                          Ajouter au panier
                        </Button>
